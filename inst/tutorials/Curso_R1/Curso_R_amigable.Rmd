...
Tratemos de ver otras operaciones directamente con ejercicios. Te invito a que generes `m1` del ejercicio anterior utilizando la función `rbind()`, `m2` utilizando la función `matrix()` y calcules el producto matricial de m1 traspuesta con m2 nombrandolo `m3`. Por último calcula la suma de la diagonal de `m3`.
...
Ahora trabajemos lo que se denomina estandarización sobre una matriz, para ello calcula la media y la desviación estándar por columnas y a cada elemento de la columna correspondiente deberas restarle la media y dividir por la desviación estándar. 
...
#Matriz de prueba
m <- matrix(sample(1:8,16,replace = T),ncol = 4,byrow = T)
#Media
mmean <- 
#Desviación estandar
msd <-
...
Para tener una mayor comprensión de la indexación en las matrices nombremos cada una de las filas y de las columnas de un matriz `m`, llamemos filas y columnas individualmente; tambien elementos individuales y por último modifiquemos algunos valores.
...
Para afianzar algunos conceptos trabajaremos sobre la matriz `m` , tendrás que calcular la suma y la media tanto por filas como por columnas, añadirlas a la matriz original utilizando `cbind`  y `rbind` y por último nombra cada una de las filas y de las columnas añadiendo  los nombres contenidos en los vectores `rname` y `cname`
...
#Matriz de prueba
m <- matrix(sample(1:16,16),ncol = 4,byrow = T)
#vectores
...
<details><summary>2.3 Operaciones con marcos de datos</summary> 

Los marcos de datos  `dataframe` son muy similares a las matrices en lo que se refiere a indexación, sin embargo su principal diferencia radica en el tipo de datos que albergan, en las matrices todos sus elementos pertenecen al mismo tipo de dato, en los dataframe cada columna puede tener un tipo de dato diferente siempre que las columnas tengan la misma longitud.  Ahora creemos un dataframe a partir vectores de diferentes tipos utilizando la función `data.frame()`.
...
logicos <- c(T,T,F,F,T,T,T,F,F,F,F,T,T,F,F)
#Dataframe
...
Podemos notar que al utilizar la función `data.frame()` el vector de letras es convertido a factor, lo que en algunos casos puede no ser conveniente, para evitar este problema podemos utilizar el argumento `stringsAsFactors = F`.
...
#Dataframe
df <- data.frame(letras,numeros,logicos,stringsAsFactors = F)
df
#Estructura
str(df)
```

También podemos crear dataframe a partir de matrices y vectores utilizando la coerción. En el siguiente ejercicio debes convertir los objetos de prueba a marcos de datos conservado sus propiedades originales, no olvides verificar su estructura.
...
mdf <- as.data.frame(m)
mdf
letrasdf <- as.data.frame(letras,stringsAsFactors = F)
letrasdf
#estructura
str(mdf)
str(letrasdf) 
...
Como vimos en el ejercicio anterior podemos acceder a una columna de un dataframe utilizando expresiones del tipo `Starwars[,1]` o `Starwars[,”name”]`, sin embargo la forma más popular y simple es utilizar la expresión `Starwars$name`. Esta última no solo nos permite acceder a una columna de nuestro set de datos, también permite crear nuevas variables. Veamos un ejemplo en el que seleccionemos la columna `species` y creemos una nueva columna que enumere nuestros datos.
...
<!--Hay muchos otros paquetes excelentes que no forman parte del tidyverse, porque están diseñados con un conjunto diferente de principios subyacentes. Esto no los hace mejores o peores, solo diferentes.-->
